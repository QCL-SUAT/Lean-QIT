...
theorem SmoothMaxRateFromMinDuality.of_all_min
  ...
  have hAB : Ω.state.marginalAB = ρ := by
    apply State.ext
    have hsw : partialTraceA (rankOneMatrix ρ.canonicalPurification.amp)
        = partialTraceB (Matrix.submatrix (rankOneMatrix ρ.canonicalPurification.amp)
    ...
    simpa [Ω, State.marginalAB_eq_marginalA, PureVector.reindex_state, State.reindex,
      ...
      PureVector.state_matrix, rankOneMatrix_apply, hsw] using
