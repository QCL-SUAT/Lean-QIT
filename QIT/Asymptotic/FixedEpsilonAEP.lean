...
private theorem canonicalAEPpurification_marginalAB
  ...
  apply State.ext
  have hbridge :
      partialTraceB (Matrix.submatrix (rankOneMatrix ρ.canonicalPurification.amp) Prod.swap Prod.swap) =
        partialTraceA (rankOneMatrix ρ.canonicalPurification.amp) := by
    ext i i'
