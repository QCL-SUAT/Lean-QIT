  ...
  · intro A B hA hB
    ...
    have hCInt :
        MeasureTheory.IntegrableOn
          ...
          (Set.Ioi 0) μ := by
      convert (hμ C hCnonneg).1 using 1 <;> rfl
    have hRightInt :
        MeasureTheory.IntegrableOn
    ...
    have hCpow :
      ...
      calc
        CFC.rpow C p = C ^ pNN := by
          have hp_eq : p = (↑pNN : ℝ) := rfl
          rw [hp_eq]
          exact (CFC.nnrpow_eq_rpow (a := C) hpNN_pos).symm
        _ = ∫ r in Set.Ioi 0, cfcₙ (Real.rpowIntegrand₁₂ p r) C ∂μ := by
          convert (hμ C hCnonneg).2 using 1; rfl
      ...
      | insert k s hks ih =>
          ...
          convert hadd using 1 <;>
            first
              ...
              | (ext r; simp [Finset.sum_insert hks, Pi.add_apply])
