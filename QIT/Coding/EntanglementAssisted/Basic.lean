...
/-- v4.34 port: explicit `rfl`-level equation lemma. The definition body is exactly this
term, but downstream `simp`/`rw` cannot unfold it (module-system opacity), so callers can
`rw [Channel.entanglementAssistedOutputState_eq]` instead. Not a `@[simp]` lemma on purpose. -/
theorem entanglementAssistedOutputState_eq {r : Type w} [Fintype r] [DecidableEq r]
    (ψ : PureVector (Prod r a)) :
    N.entanglementAssistedOutputState ψ =
      ((Channel.idChannel r).prod N).applyState ψ.state := rfl
...
/-- v4.34 port: explicit `rfl`-level equation lemma (see
`entanglementAssistedOutputState_eq`). Not a `@[simp]` lemma on purpose. -/
theorem entanglementAssistedMutualInformation_eq {r : Type w} [Fintype r] [DecidableEq r]
    (ψ : PureVector (Prod r a)) :
    N.entanglementAssistedMutualInformation ψ =
      mutualInformation (N.entanglementAssistedOutputState ψ) := rfl
