...
theorem cbOneToAlphaOriginalValue_ofState_mix_le
  ...
  have h0 :
    ...
    simpa [PhiC, hPhiC, Y0, SchattenOrder.ofPositive, SchattenOrder.ofOneLt] using
  ...
  have h1 :
    ...
    simpa [PhiC, hPhiC, Y1, SchattenOrder.ofPositive, SchattenOrder.ofOneLt] using
      cbOneToAlphaOriginalValue_eq_cpComplement_alphaToAlphaTraceValue_transpose
        Phi hPhi (lt_trans zero_lt_one halpha)
        (CBOneToAlphaOriginalDomain.ofState tau1)
  have hmix :
    ...
    simpa [PhiC, hPhiC, tauMix, Y0, Y1, CBOneToAlphaOriginalDomain.ofState,
      ...
      Matrix.transpose_smul, SchattenOrder.ofPositive, SchattenOrder.ofOneLt] using
      cbOneToAlphaOriginalValue_eq_cpComplement_alphaToAlphaTraceValue_transpose
        Phi hPhi (lt_trans zero_lt_one halpha)
        (CBOneToAlphaOriginalDomain.ofState tauMix)
  have hgoal :
...
theorem cbOneToAlphaOriginalValue_sandwichedSideWeightedMap_log_mix_min_le
  ...
  have hv0_pos : 0 < v0 := by
    simpa [Phi, hPhi, v0, SchattenOrder.ofPositive, SchattenOrder.ofOneLt] using
  ...
  have hv1_pos : 0 < v1 := by
    simpa [Phi, hPhi, v1, SchattenOrder.ofPositive, SchattenOrder.ofOneLt] using
      cbOneToAlphaOriginalValue_sandwichedSideWeightedMap_pos_of_state
        N sigma hsigma tau1 (lt_trans zero_lt_one halpha)
  have hvmix_pos : 0 < vmix := by
    simpa [Phi, hPhi, tauMix, vmix, SchattenOrder.ofPositive, SchattenOrder.ofOneLt] using
      cbOneToAlphaOriginalValue_sandwichedSideWeightedMap_pos_of_state
...
private theorem cbOneToAlphaReferenceWeight_continuousOn_posSemidef
  ...
  have hkr :
    ...
    exact _root_.continuous_matrix fun x y => by
      show Continuous ((fun T : CMatrix a1 => T x.1 y.1) *
        fun _ : CMatrix a1 => (1 : CMatrix a1) x.2 y.2)
      exact (continuous_id.matrix_elem x.1 y.1).mul continuous_const
...
private theorem cbOneToAlphaOriginalValue_sandwichedSideWeightedMap_log_continuousOn_density
  ...
  have hXcont : Continuous X := by
    ...
    simpa [S, X, Set.domRestrict_def] using
      continuousOn_iff_continuous_domRestrict.mp hcontOn
...
private theorem sandwichedChannelOriginalValueLogReferenceDensity_continuousOn_fullRank
  ...
  have hWcont : Continuous W := by
    ...
    simpa [S, W, Set.domRestrict_def] using
      continuousOn_iff_continuous_domRestrict.mp hcontOn
  ...
  have hYexplicit_cont : Continuous Yexplicit := by
    have hkr :
      ...
      unfold Matrix.kronecker
      exact _root_.continuous_matrix fun x y => by
        show Continuous ((fun _ : S => (1 : CMatrix a1) x.1 y.1) *
          fun a : S => W a x.2 y.2)
        exact continuous_const.mul ((hWcont.matrix_elem x.2 y.2))
  ...
  have hnum_pos :
      0 <
        psdSchattenPNorm
          ...
          (SchattenOrder.ofOneLt halpha) := by
    rw [hnum_eq]
    simpa [Y, MatrixMap.CBOneToAlphaOriginalDomain.ofState,
      SchattenOrder.ofPositive, SchattenOrder.ofOneLt] using
      cbOneToAlphaOriginalValue_sandwichedSideWeightedMap_pos_of_state
...
theorem sandwichedRenyiCBNormExpression_eq_sSup_stateOriginalValue_log
  ...
  have hnorm :
    ...
    have hset : Set.range v =
        Phi.cbOneToAlphaStateOriginalValueSet hPhi (SchattenOrder.ofOneLt halpha) := by
      simp only [v]
      unfold MatrixMap.cbOneToAlphaStateOriginalValueSet
      rfl
    rw [hset]
    exact MatrixMap.cbOneToAlphaNorm_eq_sSup_stateOriginalValueSet_of_one_lt
  ...
  have hpos : ∀ x ∈ Set.range v, 0 < x := by
    rintro x ⟨tau, rfl⟩
    simpa [Phi, hPhi, v, SchattenOrder.ofPositive, SchattenOrder.ofOneLt] using
      cbOneToAlphaOriginalValue_sandwichedSideWeightedMap_pos_of_state
        N sigma hsigma tau (lt_trans zero_lt_one halpha)
  have hlogSup :
    ...
    calc
      rho.sandwichedRenyiMutualInformationCandidateE sigma alpha =
          ((alpha / (alpha - 1) *
            ...
            EReal) := by
            simpa [inner, hinner, EReal.coe_div] using
