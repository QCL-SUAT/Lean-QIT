...
theorem weighted_logMoment_hasDerivAt
  ...
  convert hdiv using 1 <;>
    first | rfl | simp [S, div_eq_mul_inv, mul_comm]
...
theorem sandwichedRenyiMutualInformationCandidateE_tendsto_relativeEntropyPSDReferenceE
  ...
  · have hlim :
    ...
    have hcurve : sandwichedRenyiPSDReferenceHighAlphaCurve rhoAB sigmaAB.matrix sigmaAB.pos
        = fun alpha => sandwichedRenyiPSDReferenceE rhoAB sigmaAB.matrix sigmaAB.pos alpha.1 := rfl
    simpa [State.sandwichedRenyiMutualInformationCandidateE, hcurve, sigmaAB, hendpoint] using hlim
  · have hconst :
        Tendsto
...
theorem sandwichedRenyiMutualInformationE_upperSemicontinuousOn_of_candidate
  ...
  have h :=
    upperSemicontinuousOn_iInf
      ...
      husc
  have hfun : (fun x => (rho x).sandwichedRenyiMutualInformationE alpha)
      = fun x' => ⨅ i, (rho x').sandwichedRenyiMutualInformationCandidateE i alpha := by
    funext x
    rfl
...
theorem relativeEntropy_nonneg
  ...
  have hOut :
      (Phi.applyState rho).relativeEntropy (Phi.applyState sigma) =
        (0 : EReal) := by
    rw [hOutState, hOutSigma]
    show unit.relativeEntropyPSDReferenceTraceLogE (1 : CMatrix PUnit.{1}) _ = 0
    exact relativeEntropyPSDReferenceTraceLogE_unit_one_eq_zero
...
private theorem productLeftReference_matrix_eq_productEigenbasis_diagonal
  ...
  have hA :
    ...
    have hdiag : (fun x : a => (((BinaryHypothesisTest.stateSpectralWeight rhoA x : NNReal) : Real) : ℂ))
        = fun x : a => ((rhoA.pos.isHermitian.eigenvalues x : Real) : ℂ) := by
      funext x; rfl
    simpa [UA, hdiag, Function.comp_def, Unitary.conjStarAlgAut_apply] using
  ...
  have hB :
    ...
    have hdiag : (fun y : b => (((BinaryHypothesisTest.stateSpectralWeight sigmaB y : NNReal) : Real) : ℂ))
        = fun y : b => ((sigmaB.pos.isHermitian.eigenvalues y : Real) : ℂ) := by
      funext y; rfl
    simpa [UB, hdiag, Function.comp_def, Unitary.conjStarAlgAut_apply] using
      sigmaB.pos.isHermitian.spectral_theorem
            ...
            Matrix.diagonal
              (fun k : a =>
                (((BinaryHypothesisTest.stateSpectralWeight rhoAB.marginalA k : NNReal)
                  : Real) : ℂ)) *
            star (UA : CMatrix a) := by
      have hdiag : (fun k : a => (((BinaryHypothesisTest.stateSpectralWeight rhoAB.marginalA k : NNReal) : Real) : ℂ))
          = fun k : a => ((rhoAB.marginalA.pos.isHermitian.eigenvalues k : Real) : ℂ) := by
        funext k; rfl
      simpa [UA, hdiag, Function.comp_def, Unitary.conjStarAlgAut_apply] using hspec
    calc
      star (UA : CMatrix a) * rhoAB.marginalA.matrix * (UA : CMatrix a)
          = star (UA : CMatrix a) *
              ((UA : CMatrix a) *
  ...
  have hSigmaSpec :
      sigmaB.matrix =
        (UB : CMatrix b) *
          ...
          star (UB : CMatrix b) := by
    have hdiag : (fun j : b => (((BinaryHypothesisTest.stateSpectralWeight sigmaB j : NNReal) : Real) : ℂ))
        = fun j : b => ((sigmaB.pos.isHermitian.eigenvalues j : Real) : ℂ) := by
      funext j; rfl
    simpa [UB, nuB, hdiag, Function.comp_def, Unitary.conjStarAlgAut_apply] using
      sigmaB.pos.isHermitian.spectral_theorem
  have hlogSigma :
      cfc f sigmaB.matrix =
        (UB : CMatrix b) *
...
theorem inputSandwichedRenyiProductMarginalCandidate_tendsto_entanglementAssistedMutualInformation
  ...
  rw [Channel.entanglementAssistedMutualInformation_eq, Channel.entanglementAssistedOutputState_eq]
  exact State.sandwichedRenyiMutualInformationCandidateE_marginalB_tendsto_mutualInformation
      (N.hypothesisTestingOutputState psi)
...
theorem hypothesisTestingOutputState_continuous {r : Type u} [Fintype r] [DecidableEq r] :
  ...
  simpa [Channel.hypothesisTestingOutputState, Function.comp_def] using
...
theorem sandwichedRenyiMutualInformationE_iInf_eq_information_of_candidate_mono_and_mosonyi_hiai
  ...
  refine
    N.sandwichedRenyiMutualInformationE_iInf_eq_information_of_mosonyi_hiai
      hMosonyi ?_
  intro psi
  have hbridge : N.entanglementAssistedOutputState psi = N.hypothesisTestingOutputState psi := rfl
  rw [Channel.entanglementAssistedMutualInformation_eq, hbridge]
  exact
...
theorem sandwichedRenyiMutualInformationE_tendsto_information_of_state_limit_and_mosonyi_hiai
  ...
  rw [Channel.entanglementAssistedMutualInformation_eq, Channel.entanglementAssistedOutputState_eq]
  exact hstateEndpoint
  exact hstateTendsto psi
...
theorem exists_inputSandwichedRenyiMutualInformationE_tendsto_information_of_eventually_candidate_lower
  ...
  | Exists.intro psi hpsi =>
      ...
      have hbridge : N.entanglementAssistedOutputState psi = N.hypothesisTestingOutputState psi := rfl
      rw [hpsi, Channel.entanglementAssistedMutualInformation_eq, hbridge]
      simpa [Channel.inputSandwichedRenyiMutualInformationE] using hstate
