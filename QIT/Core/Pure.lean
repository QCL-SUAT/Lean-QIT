...
theorem rankOneMatrix_pos (ψ : a -> ℂ) :
  ...
  -- `simpa [rankOneMatrix]` no longer matches the eta-expanded `star ψ` on the
  -- lemma's side; the two sides are definitionally equal, so bridge them directly.
  have h : rankOneMatrix ψ = Matrix.vecMulVec ψ (star ψ) := rfl
  rw [h]
