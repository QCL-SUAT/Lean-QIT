...
theorem conditionalEntropy_neg_marginalA_le (τ : State (Prod a b)) :
  ...
  have hAB : Ψ₀.state.marginalA = τ := by
    apply State.ext
    have hbridge :
        partialTraceB (Matrix.submatrix (rankOneMatrix τ.canonicalPurification.amp)
          Prod.swap Prod.swap) =
        partialTraceA (rankOneMatrix τ.canonicalPurification.amp) := by
      ext i i'
      simp [partialTraceA, partialTraceB, Matrix.submatrix_apply, Prod.swap]
    simpa [Ψ₀, State.marginalA, State.marginalB, PureVector.reindex_state,
      State.reindex, QIT.partialTraceA, QIT.partialTraceB,
      PureVector.state_matrix, rankOneMatrix_apply, hbridge] using
...
theorem tendsto_binaryEntropy_nhdsWithin_zero_right :
  ...
  have hdef : binaryEntropy = fun x => -xlog2 x - xlog2 (1 - x) := rfl
  simpa [hdef] using hx.neg.sub hone
...
theorem tendsto_afwContinuityModulus_nhdsWithin_zero_right (d : ℕ) :
  ...
  have hdef : afwContinuityModulus d = fun x => 2 * x * log2 (d : ℝ) +
      (1 + x) * binaryEntropy (x / (1 + x)) := rfl
  simpa [hdef] using hlin.add hscale
...
private theorem afw_scaledLog_hasDerivAt {ε : ℝ} (hεpos : 0 < ε) :
  ...
  have hleft : HasDerivAt (fun t : ℝ => t * log2 t) (log2 ε + 1 / Real.log 2) ε := by
    ...
    convert h using 1 <;> first | rfl | simp [div_eq_mul_inv, hεpos.ne']
  have hright : HasDerivAt (fun t : ℝ => (1 + t) * log2 (1 + t))
    ...
    convert h using 1 <;> first | rfl | simp [div_eq_mul_inv, honepos.ne']
  convert hleft.neg.add hright using 1 <;> first | rfl | ring
