...
theorem hasDerivAt_chernoffPartition
  ...
  have hlin : HasDerivAt (fun t : ℝ => 1 - t) (-1) s := by
    -- rewrite the function and scalar forms explicitly; the pointwise
    -- function-subtraction form `(fun _ => 1) - id` no longer unifies directly
    have h := (hasDerivAt_const (x := s) (c := (1 : ℝ))).sub (hasDerivAt_id s)
    have hfun : ((fun _ : ℝ => (1 : ℝ)) - id) = fun t : ℝ => 1 - t := by
      funext t
      rfl
    have hder : ((0 : ℝ) - 1) = -1 := by norm_num
    rwa [hfun, hder] at h
  have hpowb :
  ...
  -- `convert` here also emits instance-equality goals; rewrite explicitly instead.
  have hfun : ((fun t : ℝ => a ^ t) * fun t : ℝ => b ^ (1 - t)) =
      fun t : ℝ => a ^ t * b ^ (1 - t) := by
    funext t
    rfl
  have hval : (Real.log a * a ^ s) * b ^ (1 - s) +
        a ^ s * (-(Real.log b * b ^ (1 - s))) =
      a ^ s * b ^ (1 - s) * (Real.log a - Real.log b) := by
    ring
