...
theorem stateSpectralWeight_sum (rho : State a) :
  ...
  have h := htrace
  simp only [stateSpectralWeight] at h ⊢
  push_cast at h ⊢
  exact h
...
theorem nussbaumSzkolaOverlap_row_sum (rho sigma : State a) (x : a) :
  ...
  apply NNReal.eq
  have h := unitary_row_normSq_sum (nussbaumSzkolaTransitionUnitary rho sigma) x
...
theorem nussbaumSzkolaOverlap_col_sum (rho sigma : State a) (y : a) :
  ...
  apply NNReal.eq
  have h := unitary_col_normSq_sum (nussbaumSzkolaTransitionUnitary rho sigma) y
  simp only [nussbaumSzkolaOverlap] at h ⊢
  push_cast at h ⊢
  exact h
...
theorem productMarginalNussbaumSzkolaOverlap_row_sum
  ...
  apply NNReal.eq
  have h := unitary_row_normSq_sum
    (productMarginalNussbaumSzkolaTransitionUnitary rhoAB) x
  simp only [productMarginalNussbaumSzkolaOverlap] at h ⊢
  push_cast at h ⊢
  exact h
