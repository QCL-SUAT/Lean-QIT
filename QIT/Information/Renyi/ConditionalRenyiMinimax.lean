...
private theorem trace_kronecker_right_continuous
  ...
  have hkr :
    ...
    exact _root_.continuous_matrix fun x y => by
      have hcont : Continuous fun a_1 : CMatrix c => K x.1 y.1 * a_1 x.2 y.2 :=
        (continuous_const.mul (continuous_id.matrix_elem x.2 y.2)).congr (fun T => rfl)
      simpa [Matrix.kroneckerMap_apply] using hcont
...
private theorem trace_kronecker_middle_continuous
  ...
  have hinner :
    ...
    unfold Matrix.kronecker
    exact _root_.continuous_matrix fun x y => by
      have hcont : Continuous fun a_1 : CMatrix b =>
          (1 : CMatrix a) x.1 y.1 * a_1 x.2 y.2 :=
        (continuous_const.mul (continuous_id.matrix_elem x.2 y.2)).congr (fun T => rfl)
      simpa [Matrix.kroneckerMap_apply] using hcont
  have hkr :
    ...
    exact _root_.continuous_matrix fun x y => by
      have hcont : Continuous fun a_1 : CMatrix b =>
          (1 : CMatrix a) x.1.1 y.1.1 * a_1 x.1.2 y.1.2 * K x.2 y.2 :=
        ((hinner.matrix_elem x.1 y.1).mul continuous_const).congr (fun T => rfl)
      simpa [Matrix.kroneckerMap_apply] using hcont
  exact Complex.continuous_re.comp
    (Continuous.matrix_trace (hkr.matrix_mul continuous_const))
