...
public theorem cMatrix_kronecker_trace_re_tendsto_right
  ...
  have hcont :
    ...
    have htrace : ContinuousAt (fun M : CMatrix b =>
      ...
      have hcont : ContinuousAt (fun M : CMatrix b => K.trace * M.trace) M :=
        ((continuous_const.mul
          (Continuous.matrix_trace continuous_id)).continuousAt).congr
            (Filter.Eventually.of_forall (fun M => rfl))
      exact hcont.congr
        (Filter.Eventually.of_forall (fun M => by simp [Matrix.trace_kronecker]))
...
public theorem cMatrix_kronecker_tendsto_right
  ...
  have hcont : Continuous fun M : CMatrix b => Matrix.kronecker K M := by
    ...
    rcases i with ⟨ia, ib⟩
    rcases j with ⟨ja, jb⟩
    have hcont : Continuous fun M : CMatrix b => K ia ja * M ib jb :=
      (continuous_const.mul (continuous_apply_apply ib jb)).congr (fun M => rfl)
    simpa [Matrix.kronecker, Matrix.kroneckerMap_apply] using hcont
  exact (hcont.tendsto B).comp hF
