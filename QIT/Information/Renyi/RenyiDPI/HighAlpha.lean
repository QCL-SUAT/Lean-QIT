...
theorem sandwichedRenyiWeightedTraceFamily_differentiable
  ...
  have hfun : (σ.sandwichedRenyiWeightedTraceFamily hσ τ hτ K Apath Bpath)
      = fun z : ℂ =>
        (cMatrixPosDefComplexPower τ.matrix hτ z *
              (MatrixMap.ofKraus K)
                (cMatrixPosDefComplexPower σ.matrix hσ (-z) * Apath z *
                  cMatrixPosDefComplexPower σ.matrix hσ (-z)) *
            cMatrixPosDefComplexPower τ.matrix hτ z *
          Bpath z).trace := by
    funext z
...
theorem sandwichedRenyiBeigiInputPath_differentiable
  ...
  have hfun : (sandwichedRenyiBeigiInputPath A hA α)
      = fun z : ℂ => cMatrixPosDefComplexPower A hA (((2 * α : ℝ) : ℂ) * z + (α : ℂ)) := by
    funext z
    simp only [sandwichedRenyiBeigiInputPath]
    congr 1
    push_cast
    ring
  rw [hfun]
  exact cMatrixPosDefComplexPower_affine_differentiable hA ((2 * α : ℝ) : ℂ) (α : ℂ)
...
theorem sandwichedRenyiBeigiDualPath_differentiable
  ...
  have hfun : (sandwichedRenyiBeigiDualPath B hB q)
      = fun z : ℂ => cMatrixPosDefComplexPower B hB (((-2 * q : ℝ) : ℂ) * z + 0) := by
    funext z
    simp only [sandwichedRenyiBeigiDualPath]
    congr 1
    push_cast
    ring
  rw [hfun]
  exact cMatrixPosDefComplexPower_affine_differentiable hB ((-2 * q : ℝ) : ℂ) 0
...
theorem sandwichedRenyiBeigiNormalizedInputPath_differentiable
  ...
  have hfun : (sandwichedRenyiBeigiNormalizedInputPath A hA α C)
      = fun z : ℂ =>
          Complex.exp ((1 - (2 * (α : ℂ) * z + (α : ℂ))) * ((Real.log C : ℝ) : ℂ)) •
            cMatrixPosDefComplexPower A hA (2 * (α : ℂ) * z + (α : ℂ)) := by
    funext z
    simp only [sandwichedRenyiBeigiNormalizedInputPath]
  rw [hfun]
  ...
  calc
    MatrixMap.ofKraus (sandwichedRenyiRotatedKrausComplex σ hσ τ hτ K z)
        ...
        T * MatrixMap.ofKraus K
          ...
          have hkr : (sandwichedRenyiRotatedKrausComplex σ hσ τ hτ K z)
              = fun k => T * K k * S := by
            funext k
            simp only [sandwichedRenyiRotatedKrausComplex, T, S]
          rw [hkr]
