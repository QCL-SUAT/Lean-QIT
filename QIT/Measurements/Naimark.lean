...
theorem familyNaimark_compression_projector_eq [Inhabited y]
  ...
  have hemb : familyNaimarkEmbedding M = (M setting).fixedNaimarkEmbedding := rfl
  have hpm : familyNaimarkProjectiveMeasurement M setting =
      (M setting).fixedNaimarkProjectiveMeasurement := rfl
  rw [hemb, hpm]
  exact (M setting).fixedNaimark_compression_projector outcome
...
theorem familyNaimark_prob_eq [Inhabited y]
  ...
  have hpm : familyNaimarkProjectiveMeasurement M setting =
      (M setting).fixedNaimarkProjectiveMeasurement := rfl
  have hstate : familyNaimarkLiftState M rho = (M setting).fixedNaimarkLiftState rho := rfl
  rw [hpm, hstate]
  exact (M setting).fixedNaimark_prob_eq rho outcome
