    ...
    have hblock_nonneg :
      ...
      have htrace_nonneg : 0 ≤ (Classical.block ρ.matrix x x).trace :=
        Matrix.PosSemidef.trace_nonneg
          (by
            have hb : Classical.block ρ.matrix x x =
                Matrix.submatrix ρ.matrix (fun i : b => (x, i)) (fun i : b => (x, i)) := rfl
            rw [hb]
      ...
      State.normalizePSD (Classical.block ρ.matrix x x)
        (by
          have hb : Classical.block ρ.matrix x x =
              Matrix.submatrix ρ.matrix (fun i : b => (x, i)) (fun i : b => (x, i)) := rfl
          rw [hb]
          exact ρ.pos.submatrix (fun i : b => (x, i))) hx
...
private theorem sourceCoordinatePinchEnsemble_weightedBlock
    ...
    have hBpos : B.PosSemidef := by
      have hb : B = Matrix.submatrix ρ.matrix (fun i : b => (x, i))
          (fun i : b => (x, i)) := rfl
      rw [hb]
      exact ρ.pos.submatrix (fun i : b => (x, i))
...
theorem cqPrimalProgram_dualValueSet_subset_cqDualValueSet (E : Ensemble ι b) :
  ...
  have hrep_psd (A : CMatrix b) (hA : A.PosSemidef) :
    ...
    have h := hTrep X
    have hb : (X : CMatrix b) = A := rfl
    have hX : hermitianInclusionNormed X = A := rfl
    simpa [yH, ContinuousLinearMap.comp_apply, tracePairing, hX, hb] using h
  refine ⟨T.val, ?_, ?_⟩
  · constructor
  ...
  · have hrep_one : y (1 : CMatrix b) = E.cqDualValue T.val := by
      ...
      have h := hTrep X
      have hb : (X : CMatrix b) = 1 := rfl
      have hX : hermitianInclusionNormed X = (1 : CMatrix b) := rfl
      simpa [yH, ContinuousLinearMap.comp_apply, tracePairing, cqDualValue, hX, hb] using h
