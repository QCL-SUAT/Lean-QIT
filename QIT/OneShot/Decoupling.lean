...
private theorem unitaryTwirlIntegrand_oneCopyMatrixLift_apply [Fintype a] [DecidableEq a]
  ...
  rw [sum_tensorPower_one]
  have hstar :
      star (Matrix.kronecker (U : CMatrix a) (1 : CMatrix PUnit.{u+1}))
          (x, PUnit.unit) (j, PUnit.unit) = star ((U : CMatrix a) j x) := by
    simp [Matrix.star_apply, Matrix.kronecker]
  change (∑ i_1 : a,
        (U : CMatrix a) i ((decouplingTensorPowerOneEquiv a).symm i_1).1 *
          (1 : CMatrix PUnit) (i, PUnit.unit).2
            ((decouplingTensorPowerOneEquiv a).symm i_1).2 *
          M ((decouplingTensorPowerOneEquiv a).symm i_1).1 x) *
