...
private theorem referenceIsometry_applyMatrix_kronecker_one_right_le
  ...
  have heq : Matrix.kronecker (1 : CMatrix aPlus) T -
      Matrix.kronecker (V.matrix * Matrix.conjTranspose V.matrix) T =
      Matrix.kronecker ((1 : CMatrix aPlus) - V.matrix * Matrix.conjTranspose V.matrix) T := by
    ext x y
...
theorem ConditionalMinEntropyScaleFeasible.compress_sumInr
  ...
  · have hsub := hT.2.submatrix (fun x : Prod a b => (x.1, Sum.inr x.2))
    change (Matrix.kronecker (1 : CMatrix a) (Matrix.sumBlock22 TPlus) -
      ρ.matrix).PosSemidef
    have heq : Matrix.kronecker (1 : CMatrix a) (Matrix.sumBlock22 TPlus) - ρ.matrix =
        (Matrix.kronecker (1 : CMatrix a) TPlus -
          (ρ.conditioningIsometryApply (ReferenceIsometry.sumInr extra b)).matrix).submatrix
          (fun x : Prod a b => (x.1, Sum.inr x.2))
          (fun x : Prod a b => (x.1, Sum.inr x.2)) := by
      ext x y
  ...
  ofClassicalBlocks
    ...
    (by
      ...
      exact Matrix.posSemidef_sum Finset.univ fun x _ => by
        by_cases hx : g x = y
        · have h := ρ.pos.submatrix (fun i : b => (x, i))
          have hfun : ρ.matrix.submatrix (fun i : b => (x, i)) (fun i : b => (x, i)) =
              Classical.block ρ.matrix x x := by
            ext i j
            simp [Classical.block, Matrix.submatrix_apply]
          rw [hfun] at h
          simpa [hx] using h
...
theorem ConditionalMinEntropyScaleFeasible.ofStateScale
  ...
  · have hscaled := cMatrix_real_smul_le_smul (A := ρ.matrix)
    ...
    have h1 : (SubnormalizedState.ofStateScale ρ t ht0 ht1).matrix = t • ρ.matrix := rfl
    have h2 : Matrix.kronecker (1 : CMatrix a) (t • T) =
        t • Matrix.kronecker (1 : CMatrix a) T := by
      ext i j
      simp [Matrix.kronecker, Matrix.kroneckerMap_apply, Complex.real_smul, mul_assoc,
