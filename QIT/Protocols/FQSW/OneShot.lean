...
theorem adhwFQSW_traceNormSqBound_of_HSOneShotExact
  ...
  have heq : dq * de * dr / dq ^ 2 * (X + Y) = de * dr * (1 / dq * (X + Y)) := by
    field_simp [ne_of_gt hdq_pos]
  rwa [heq]
...
theorem adhwFQSW_source_component_bounds_le_oneShotFourthRootArgument
  ...
  have hmaxMixed_absorbed :
    ...
    have h1 : ↑(Fintype.card a) / ↑(Fintype.card q) ^ 2 * adhwFQSWAPurity ψ =
        ↑(Fintype.card a) / ↑(Fintype.card q) ^ 2 * 1 * adhwFQSWAPurity ψ := by
      ring
    have h2 : ↑(Fintype.card a) * ↑(Fintype.card r) / ↑(Fintype.card q) ^ 2 *
        adhwFQSWAPurity ψ * adhwFQSWRPurity ψ =
        ↑(Fintype.card a) / ↑(Fintype.card q) ^ 2 * (↑(Fintype.card r) *
          adhwFQSWRPurity ψ) * adhwFQSWAPurity ψ := by
      ring
    rw [h1, h2]
    exact hmulA
