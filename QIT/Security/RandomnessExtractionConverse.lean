...
private theorem fiberGram_eq_mul (Y : Matrix β (Prod ι β) ℂ) (i : ι) :
    fiberGram Y i = fiberMatrix Y i * (fiberMatrix Y i)ᴴ := by
  unfold fiberGram fiberMatrix
  rfl
...
private theorem fiber_gram_pos
  ...
  let Yi : CMatrix β := fun x y => Y x (i, y)
  have hYi : fiberGram Y i = Yi * Yiᴴ := by
    unfold fiberGram
    rfl
  rw [hYi]
...
private theorem exists_fiber_psd_split_traceNorm_bound
  ...
  · calc
      ...
      _ ≤ ∑ i : ι, traceNorm (psdSqrt (A i) * psdSqrt (fiberGram Y i)) := by
        ...
        have hle := abs_trace_psdSqrt_mul_amplitude_le (A i) (hA i) (fiberMatrix Y i)
        rw [fiberGram_eq_mul Y i]
        exact hle
...
theorem conditionalMinEntropyFeasible_of_seededSourceEnsemble
  ...
  have hsum_le :
    ...
    have hsmul : Classical.block (c • σ.matrix) f f =
        c • Classical.block σ.matrix f f := by
      ext i j
      simp only [Classical.block, Matrix.smul_apply]
    rw [hsmul] at hblock
    exact hblock
