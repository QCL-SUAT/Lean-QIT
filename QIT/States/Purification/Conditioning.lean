...
theorem conditioningSupportCompressedState_conditioningIsometryApply
  ...
  have hbr : ReferenceIsometry.rightBlock
        (psdSupportCompressRight (a := a) (partialTraceA ρ.matrix) hN ρ.matrix) x.1 y.1 =
      (psdSupportIsometry (partialTraceA ρ.matrix) hN)ᴴ *
        ReferenceIsometry.rightBlock ρ.matrix x.1 y.1 *
        psdSupportIsometry (partialTraceA ρ.matrix) hN := by
    ext i j
    simp [psdSupportCompressRight, ReferenceIsometry.rightBlock]
  simpa [conditioningIsometryApply_matrix, ReferenceIsometry.applyMatrixRight,
    ...
    ReferenceIsometry.rightBlock, psdSupportCompress, B, N, hN, hbr] using hentry
