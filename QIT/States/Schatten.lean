...
theorem unitary_row_normSq_sum (U : Matrix.unitaryGroup a ℂ) (i : a) :
  ...
  simpa [Matrix.mul_apply, Matrix.one_apply, Complex.normSq_apply, Complex.normSq_eq_conj_mul_self,
    mul_comm] using hre
...
theorem posSemidef_diagonal_re_eq_eigenvalue_weighted_sum
  ...
  simpa [U, D, Matrix.mul_apply, Matrix.diagonal, Complex.normSq_apply, Complex.normSq_eq_conj_mul_self,
...
theorem cMatrixPosDefComplexPower_differentiable
  ...
  have hfun : (∑ x, fun z : ℂ => (∑ y, (U : CMatrix a) i y * D z y x) *
        star (U : CMatrix a) x j) =
      (fun z : ℂ => ∑ x, (∑ y, (U : CMatrix a) i y * D z y x) *
        star (U : CMatrix a) x j) := by
    funext z
  ...
  have hptB :
    ...
    have hdiag :
      ...
      have hρA :
        ...
        simpa [UA, dA, Matrix.IsHermitian.spectral_theorem, Unitary.conjStarAlgAut_apply, Function.comp_def]
          using hspec
      calc
        star (UA : CMatrix a) * ρA.matrix * (UA : CMatrix a)
            = star (UA : CMatrix a) *
  ...
  have hptA :
    ...
    have hdiag :
      ...
      have hρB :
        ...
        simpa [UB, dB, Matrix.IsHermitian.spectral_theorem, Unitary.conjStarAlgAut_apply, Function.comp_def]
          using hspec
      calc
        star (UB : CMatrix b) * ρB.matrix * (UB : CMatrix b)
            = star (UB : CMatrix b) *
...
theorem posSemidef_unitary_conj {A : CMatrix a} (hA : A.PosSemidef)
  ...
  simpa [Matrix.mul_assoc, Matrix.star_eq_conjTranspose] using hA.conjTranspose_mul_mul_same (U : CMatrix a)
...
theorem cMatrix_rpow_add_nonneg_smul_one_eigenbasis_diagonal
  ...
  have hA_spec :
    ...
    simpa [U, d, D, Matrix.IsHermitian.spectral_theorem, Function.comp_def,
...
theorem psdSupportIsometry_mul_conjTranspose_eq_supportProjector
  ...
  have hsub :
    ...
    have h1 : (∑ x : psdSupportIndex N hN, f x.1) =
        ∑ x ∈ (Finset.univ : Finset a) with 0 < hN.isHermitian.eigenvalues x, f x := by
      rw [← Finset.sum_subtype_eq_sum_filter (s := (Finset.univ : Finset a))
        (p := fun x => 0 < hN.isHermitian.eigenvalues x) (f := f)]
      congr 1
      ext x
      simp
    simpa [f] using h1
...
theorem psdSchattenPNorm_tendsto_of_tendsto_posSemidef
  ...
  simpa [psdSchattenPNorm, Internal.psdSchattenExpression, Function.comp_def] using
...
theorem psdTraceReverseHolderOptimizer_props
  ...
  have hN : N.PosSemidef := by
    ...
    simpa [N, Matrix.star_eq_conjTranspose] using hdiag.mul_mul_conjTranspose_same (U : CMatrix a)
...
theorem psdTraceHolderUnitBall_isGreatest
  ...
  · let U : Matrix.unitaryGroup a ℂ := hM.isHermitian.eigenvectorUnitary
    ...
    have hB : B.PosSemidef := by
      simpa [B, Matrix.star_eq_conjTranspose] using hD.mul_mul_conjTranspose_same (U : CMatrix a)
