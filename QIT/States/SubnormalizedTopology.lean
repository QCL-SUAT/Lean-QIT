...
theorem subnormalizedMatrixSet_isClosed :
  ...
  have hpsd : IsClosed ({M : CMatrix a | M.PosSemidef} : Set (CMatrix a)) := by
    have h := (psdCone a).isClosed
    have hset : ((psdCone a : Set (CMatrix a)) : Set (CMatrix a)) =
        {M : CMatrix a | M.PosSemidef} := by
      ext M
      simp [psdCone, psdSubmodule]
    rwa [hset] at h
  have htrace :
...
theorem continuous_generalizedFidelity_right (ρ : SubnormalizedState a) :
  ...
  exact ((hnorm.add hsqrt).pow 2).congr (fun σ => by simp [generalizedFidelity])
...
theorem psdTraceBoundedMatrixSet_isClosed (R : ℝ) :
  ...
  have hpsd : IsClosed ({T : CMatrix b | T.PosSemidef} : Set (CMatrix b)) := by
    have h := (psdCone b).isClosed
    have hset : ((psdCone b : Set (CMatrix b)) : Set (CMatrix b)) =
        {M : CMatrix b | M.PosSemidef} := by
      ext M
      simp [psdCone, psdSubmodule]
    rwa [hset] at h
  have htrace :
