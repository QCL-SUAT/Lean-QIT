...
public theorem cMatrix_rpow_mul_rpow_of_pos_add_eq_one {A : CMatrix a}
  ...
  have hrpow : A ^ rNN = CFC.rpow A r := by
    have h := CFC.nnrpow_eq_rpow (a := A) hrNN
    have hexp : ((rNN : ℝ)) = r := rfl
    rwa [hexp] at h
  have hspow : A ^ sNN = CFC.rpow A s := by
    have h := CFC.nnrpow_eq_rpow (a := A) hsNN
    have hexp : ((sNN : ℝ)) = s := rfl
    rwa [hexp] at h
  calc
...
public theorem audenaertResolventPath_hasDerivAt {A B : CMatrix a}
  ...
  have hfun : ((fun s : ℝ => audenaertPathMatrix A B s) * fun s : ℝ =>
        (audenaertPathMatrix A B s + r • (1 : CMatrix a))⁻¹) =
      (fun s : ℝ => audenaertPathMatrix A B s *
        (audenaertPathMatrix A B s + r • (1 : CMatrix a))⁻¹) := by
    funext s
    rfl
  rw [hfun] at hprod'
  simpa [C, X, V, audenaertPathResolvent] using hprod'
...
public theorem audenaertLemma4_interior_trace_nonneg {t : ℝ≥0}
  ...
  have hf : IntegrableOn f (Set.Ioi 0) μ := by
    have h := hAint.sub hBint
    have hfun : ((fun r => audenaertRpowIntegrand (t : ℝ) r A) -
        fun r => audenaertRpowIntegrand (t : ℝ) r B) = f := by
      funext r
      simp [f]
    rwa [hfun] at h
...
public theorem audenaertTraceInequality_leftHalf {s : ℝ}
  ...
  have hXpow : CFC.rpow X (tNN : ℝ) = CFC.rpow A s := by
    have h := cMatrix_rpow_rpow_of_nonneg hA hrPos.le ht0 hrt
    have hexp : ((tNN : ℝ)) = t := rfl
    rw [hexp]
    simpa [X] using h
  have hYpow : CFC.rpow Y (tNN : ℝ) = CFC.rpow B s := by
    have h := cMatrix_rpow_rpow_of_nonneg hB hrPos.le ht0 hrt
    have hexp : ((tNN : ℝ)) = t := rfl
    rw [hexp]
    simpa [Y] using h
