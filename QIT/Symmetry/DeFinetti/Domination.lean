...
theorem symmetricProjectionReferenceState_supportedOnSymmetricSubspace
  ...
  simpa [hscale, sub_smul, Matrix.smul_apply, symmetricProjectionReferenceState] using
    (symmetricProjectionMatrix_posSemidef (a := a) n).smul hscale
...
theorem matrixDominatedBy_trans {ρ σ τ : State a} {c d : ℝ}
  ...
  have heq : ((c * d : ℝ) : ℂ) • τ.matrix - (c : ℂ) • σ.matrix =
      (c : ℂ) • (d • τ.matrix - σ.matrix) := by
    ext i j
    simp only [Matrix.sub_apply, Matrix.smul_apply, Complex.real_smul, Complex.ofReal_mul]
    ring
  rw [heq]
  exact hστ.smul (by exact_mod_cast hc : (0 : ℂ) ≤ (c : ℂ))
