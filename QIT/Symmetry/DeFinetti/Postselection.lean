...
private theorem dropRightUnitMatrix_kron_id_traceEffectToUnit_apply
    ...
    dropRightUnitMatrix
      ...
      ∑ p : β, ∑ q : β, X (x, p) (y, q) * E q p := by
  classical
  simpa [dropRightUnitMatrix, MatrixMap.kron, idChannel_map_eq_self] using
...
private theorem classical_block_posSemidef {ι : Type w} {β : Type x}
  ...
  change (M.submatrix (fun x : β => (i, x)) (fun x : β => (i, x))).PosSemidef
...
theorem ckrProfileCoordinateMatrix_le_one [Nonempty a]
  ...
  have heq : 1 - ρ.ckrProfileCoordinateMatrix (a := a) =
      U.conjTranspose *
...
theorem ckrProfileCoordinateEffect_le_one [Nonempty a]
  ...
  have heq : (1 - ρ.ckrProfileCoordinateMatrix (a := a)).transpose =
      1 - (ρ.ckrProfileCoordinateMatrix (a := a)).transpose := by
