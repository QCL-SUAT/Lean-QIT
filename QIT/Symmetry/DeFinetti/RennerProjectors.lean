  ...
  calc
    ...
    _ = ψ i * star (ψ j) := by
            rw [show ∑ k, P i k * ψ k = ψ i by
              simpa [Matrix.mulVec, dotProduct] using congrFun hψ i]
    ...
    have hconj : (∑ k, P k j * star (ψ k)) = star (ψ j) := by
      have hconj := congrArg star
        (show ∑ k, P j k * ψ k = ψ j by
          simpa [Matrix.mulVec, dotProduct] using congrFun hψ j)
...
private theorem pure_prod_zero_reindex_state_eq_tensorPower_reindex
  ...
  have hA : (ν.tensorPower m).amp ((tensorPowerTakeDropEquiv a m 0) x).1 =
      (ν.tensorPower m).amp x :=
    congrArg (fun z => (ν.tensorPower m).amp z) hx
  have hB : (ν.tensorPower m).amp ((tensorPowerTakeDropEquiv a m 0) y).1 =
      (ν.tensorPower m).amp y :=
    congrArg (fun z => (ν.tensorPower m).amp z) hy
  have hη : η.amp ((tensorPowerTakeDropEquiv a m 0) x).2 *
      star (η.amp ((tensorPowerTakeDropEquiv a m 0) y).2) = 1 := by
    have hsub : ((tensorPowerTakeDropEquiv a m 0) x).2 =
        ((tensorPowerTakeDropEquiv a m 0) y).2 := Subsingleton.elim _ _
    rw [hsub]
    have h := η.trace_rankOne_eq_one
    rw [rankOneMatrix_trace] at h
    rw [dotProduct] at h
    rw [Fintype.sum_subsingleton (fun z => η.amp z * star (η.amp z))
      ((tensorPowerTakeDropEquiv a m 0) x).2] at h
    exact h
  calc
    (ν.tensorPower m).amp ((tensorPowerTakeDropEquiv a m 0) x).1 *
        η.amp ((tensorPowerTakeDropEquiv a m 0) x).2 *
        (star ((ν.tensorPower m).amp ((tensorPowerTakeDropEquiv a m 0) y).1) *
          star (η.amp ((tensorPowerTakeDropEquiv a m 0) y).2))
      = (ν.tensorPower m).amp x * η.amp ((tensorPowerTakeDropEquiv a m 0) x).2 *
          (star ((ν.tensorPower m).amp ((tensorPowerTakeDropEquiv a m 0) y).1) *
            star (η.amp ((tensorPowerTakeDropEquiv a m 0) y).2)) :=
        congrArg (fun t => t * η.amp ((tensorPowerTakeDropEquiv a m 0) x).2 *
          (star ((ν.tensorPower m).amp ((tensorPowerTakeDropEquiv a m 0) y).1) *
            star (η.amp ((tensorPowerTakeDropEquiv a m 0) y).2))) hA
    _ = (ν.tensorPower m).amp x * η.amp ((tensorPowerTakeDropEquiv a m 0) x).2 *
          (star ((ν.tensorPower m).amp y) *
            star (η.amp ((tensorPowerTakeDropEquiv a m 0) y).2)) :=
        congrArg (fun t => (ν.tensorPower m).amp x *
          η.amp ((tensorPowerTakeDropEquiv a m 0) x).2 *
          (star t * star (η.amp ((tensorPowerTakeDropEquiv a m 0) y).2))) hB
    _ = (ν.tensorPower m).amp x *
          (star ((ν.tensorPower m).amp y) *
            (η.amp ((tensorPowerTakeDropEquiv a m 0) x).2 *
              star (η.amp ((tensorPowerTakeDropEquiv a m 0) y).2))) := by
        ring
...
theorem PureVector.tensorPower_reindex_addZero_isRennerMIIDIn_zero
  ...
  rw [show (permEquiv (a := a) (m + 0) 1).symm = Equiv.refl (TensorPower a (m + 0)) from by
      rw [permEquiv_one]
      rfl,
    State.reindex_refl]
...
private theorem matrix_le_one (ρ : State a) :
  ...
  have hdiag : ρ.matrix = (U : CMatrix a) * D * star (U : CMatrix a) := by
    simpa [U, D, Matrix.IsHermitian.spectral_theorem, Unitary.conjStarAlgAut_apply, Function.comp_def]
...
private theorem rennerMIIDProjectorZero_integrable
  ...
  have h := unitaryTwirl_integrand_integrable (a := a) m
    (rankOneMatrix (ν.tensorPower m).amp)
  have hfun : (fun U : Matrix.unitaryGroup a ℂ =>
        rennerMIIDProjectorZero (a := a) m ν U) =
      unitaryTwirlIntegrand (a := a) m (rankOneMatrix (ν.tensorPower m).amp) := by
    funext U
    exact rennerMIIDProjectorZero_eq_unitaryTwirlIntegrand (a := a) m ν U
  rw [hfun]
  exact h
...
private theorem rennerRhoUMatrix_continuous [Nonempty a] {n k : ℕ}
  ...
  exact (hPT.const_smul (((Fintype.card (TensorPowerProfile a k) : ℝ) : ℂ))).congr
    (fun U => rfl)
...
private theorem psdSqrt_permutationChannel_map {n : ℕ}
  ...
  simpa [psdSqrt, CFC.sqrt_eq_rpow, U, Matrix.star_eq_conjTranspose,
    permutationMatrix, permutationMatrix_conjTranspose, Equiv.Perm.inv_def,
    Matrix.mul_assoc] using hpow
