...
theorem twoLevelRotationEntry_hasDerivAt_zero {i j : a} (hij : i ≠ j) (x y : a) :
  ...
  · by_cases h2 : x = i ∧ y = j
    · rcases h2 with ⟨rfl, rfl⟩
      ...
      have h := hsin.neg
      have hfun : (-fun y : ℝ => Complex.sin ↑y) = fun θ : ℝ => -Complex.sin ↑θ := by
        funext t
        rfl
      rw [hfun] at h
      simpa using h
...
private theorem integral_matrix_conjTranspose [Nonempty a]
  ...
  have hcomp : ∀ A : CMatrix ι, cMatrixConjTransposeCLM A = A.conjTranspose := fun A => rfl
  simpa only [hcomp, Function.comp_def] using
