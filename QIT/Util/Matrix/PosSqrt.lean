...
-- NOTE: the entrywise `algebraMap` reductions below rely on implicit-argument defeq
-- checks unfolding `Matrix`; `backward.isDefEq.respectTransparency = false` is set
-- library-wide in the lakefile (see the comment there).
